...
def get_parser():
    ...
    parser.add_argument('-df', '--df_path', type=str, default='airbnb/airbnb_process_data/airbnb_process_train.csv')
    parser.add_argument('-dfv', '--df_valid_path', type=str, default='airbnb/airbnb_process_data/airbnb_process_valid.csv')
    parser.add_argument('--df_test_path', type=str, default='airbnb/airbnb_process_data/airbnb_process_test.csv')
    parser.add_argument('--train_num_df', type=str, default='airbnb/airbnb_process_data/train_num.csv')
    parser.add_argument('--valid_num_df', type=str, default='airbnb/airbnb_process_data/valid_num.csv')
    parser.add_argument('--test_num_df', type=str, default='airbnb/airbnb_process_data/test_num.csv')
    parser.add_argument('--train_categ_df', type=str, default='airbnb/airbnb_process_data/train_categ.csv')
    parser.add_argument('--valid_categ_df', type=str, default='airbnb/airbnb_process_data/valid_categ.csv')
    parser.add_argument('--test_categ_df', type=str, default='airbnb/airbnb_process_data/test_categ.csv')
    ...
    parser.add_argument('-img', '--img_dir', type=str, default='airbnb/airbnb_png')
    parser.add_argument('--output_dir', type=str, default='airbnb/airbnb_emb_img_before_trans')
...
class AirbnbDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Image
        img_name = os.path.join(self.img_dir, str(self.df.iloc[idx]['id'])+'.png')
        try:
        ...
        text = "[CLS] " + str(self.df.iloc[idx]['description'])+str(self.df.iloc[idx]['neighborhood_overview'])+str(self.df.iloc[idx]['host_about']) + " [SEP] "
        ...
        label = label = torch.tensor(np.log(self.df.iloc[idx]['label']), dtype=torch.float)
...
class MultimodalModel(nn.Module):
    def __init__(self,categories=((2,) * 68),num_continuous= 17, pretrained_text_model = "allenai/longformer-base-4096",
...
if __name__ == '__main__':
    ...
    train_set = AirbnbDataset(train_df , img_dir, train_categ_df, train_num_df, pretrained_text_model=pretrained_text_model, pretrained_image_model= pretrained_image_model)
    valid_set = AirbnbDataset(valid_df , img_dir, valid_categ_df, valid_num_df,pretrained_text_model=pretrained_text_model, pretrained_image_model= pretrained_image_model)
    test_set = AirbnbDataset(test_df , img_dir, test_categ_df, test_num_df,pretrained_text_model=pretrained_text_model, pretrained_image_model= pretrained_image_model)
