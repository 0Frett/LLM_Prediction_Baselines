...
if __name__ == '__main__':
    ...
    best_loss = 100000000000000000000000000000000000000.11
    train_losses_his = []
    valid_losses_his = []
    early_stop = 0
    for epoch in range(num_epoch):
        ...
        print(f"Epoch {epoch + 1} | loss = {(sum(train_loss) / len(train_loss)):.3f}")
        ...
        if validation:
            ...
            with torch.no_grad():
                ...
                print(f"Validation | Epoch {epoch + 1} | loss = {(sum(valid_loss) / len(valid_loss)):.3f}")
                valid_losses_his.append((sum(valid_loss)/ len(valid_loss)))
            model.train()

            if best_loss > (sum(valid_loss) / len(valid_loss)):
                early_stop = 0
